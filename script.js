...
function load()
{
    ...
    for (let i = 1; i <= paddingDays + daysInMonth; i++) {
        ...
        if (i > paddingDays) {
           ...
           let eventForDay = events.find(e => e.date === dayString);  //check if event is present for current day

           //highlight current day in month
           ...
           if (eventForDay) {
               ...
               eventDiv.innerText = eventForDay.title;  //set text for event
...
//filter out the single event which is to be deleted
